...
| Branch / commit                     | `spike/sp5-notifications` @ `ea1b5144736dc0f12970fb05d581679c3207787d` (base commit the harness was built on top of; the harness itself — `src-tauri/Cargo.toml` feature additions, `src-tauri/src/spike/`, `SPIKE-SP5.md` — is **uncommitted** in the worktree per this task's "do not commit/push" instruction; the owner reviews and commits it, or reproduces it from `SPIKE-SP5.md`, before running the scenarios below) | same                                  |
...
- Dev mode (`pnpm tauri dev`) cannot be used to produce a valid S2–S7 result on
  either OS for `user-notify` or `notify-rust` (`preview-macos-un`) — both fall
  back to a mock/no-op path without a proper bundle id (macOS) or AUMID (Windows).
  Use the packaged-build instructions in `SPIKE-SP5.md`.
